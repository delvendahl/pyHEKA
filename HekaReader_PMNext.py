...
class Bundle(object):
    ...
    def __init__(self, file_name):
        ...
        # check if version number is 1.6.0 or above, which have a different header structure
        self.fh.seek(8)
        version = self.fh.read(32).decode('utf-8').rstrip('\0')
        # get first three characters of version string, which should be enough to determine the header structure
        if version[0:3] == 'v2x':
            self.file_format = 'PM'
        elif version[0:2] == '1.':
            if int(version[2]) >= 6:
                self.file_format = 'PMNext'
            else:
                self.file_format = 'PM'
